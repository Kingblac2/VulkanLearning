...
{
	glm::vec2 new_vertex = glm::vec2(std::stof(words[1]), std::stof(words[2]));
	vt.push_back(new_vertex);
}

void vkMesh::ObjMesh::read_normal_data(const std::vector<std::string>& words)
{
	glm::vec4 new_vertex = glm::vec4(std::stof(words[1]), std::stof(words[2]), std::stof(words[3]), 0.0f);
	glm::vec3 transformed_vertex = glm::vec3(preTransform * new_vertex);
	vn.push_back(transformed_vertex);
}
...
void vkMesh::ObjMesh::read_corner(const std::string& vertex_description)
{
	...
	if (v_vt_vn.size() == 3 && v_vt_vn[1].size() > 0) {
		texcoord = vt[std::stoi(v_vt_vn[1]) - 1];
	}
